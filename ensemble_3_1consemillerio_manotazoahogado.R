...
PARAM <- list()
PARAM$experimento <- "ensemble_3_modelos_1_consemillerio"


# Leer los tres archivos
archivo1 <- read.csv("~/buckets/b1/exp/intento-lightgbm-mix-1-SEMILLERIO_PLUS-promedios/intento-lightgbm-mix-1-SEMILLERIO_PLUS-promedios_predicciones_semillas.csv", header = TRUE)
...
archivo1 <- archivo1[, c("numero_de_cliente", "foto_mes", "promedio"), drop = FALSE]


if (!all(names(archivo1)[1:2] == names(archivo2)[1:2] & names(archivo2)[1:2] == names(archivo3)[1:2])) {
...
predicciones <- data.frame(
  archivo1[, 1:2],
  Promedio = rowMeans(cbind(archivo1[, 3], archivo2[, 3], archivo3[, 3]), na.rm = TRUE)
)
